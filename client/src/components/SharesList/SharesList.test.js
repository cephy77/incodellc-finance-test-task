...
test('ShareList should render items', () => {
  const screen = render(
...
test('ShareList should render proper number of items', () => {
  const screen = render(
    <Provider store={store}>
      <SharesList />
    </Provider>
  ...
  expect(elements.length === 6).toBe(true);
});

test('ShareList should has a message if there is no stocks', () => {
  const store = mockStore({
    sharesList: {
      shares: []
    },
  ...
  const element = screen.getByText('The stocks are loading');
